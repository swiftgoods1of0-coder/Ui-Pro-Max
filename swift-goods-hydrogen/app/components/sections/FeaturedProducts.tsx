    ...
    <Link to={`/products/${product.handle}`} style={{ display: 'block' }}>
      ...
      >
        ...
        <div className="relative overflow-hidden" style={{ aspectRatio: '4/5', background: '#0a0a0a' }}>
          ...
          <div className="absolute inset-0 pointer-events-none transition-opacity duration-500 opacity-0 group-hover:opacity-100" style={{ background: 'rgba(5,5,5,0.58)' }} />
        ...
        <div className="p-6 flex flex-col gap-1 flex-1">
          ...
          <h3 className="mt-1 leading-snug" style={{ fontFamily: 'var(--font-display, "Cormorant Garamond", serif)', fontSize: '1.3rem', fontStyle: 'italic', color: '#f5f5f5' }}>
            {product.title}
          </h3>
          <div className="flex items-baseline gap-3 mt-2">
            <span style={{ fontFamily: 'var(--font-impact, "Bebas Neue", sans-serif)', fontSize: '1.4rem', letterSpacing: '0.06em', background: 'linear-gradient(135deg, #c9a84c, #e6c870, #c8aa8a)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              ${product.price}
            </span>
            {product.compareAtPrice && (
              <span className="line-through" style={{ fontFamily: 'var(--font-body, Inter, sans-serif)', fontSize: '0.875rem', color: '#444444' }}>
                ${product.compareAtPrice}
              </span>
            )}
          </div>
...
export default function FeaturedProducts({ products }: FeaturedProductsProps) {
  ...
  return (
    <section ref={sectionRef} id="collection" className="relative py-24 overflow-hidden" style={{ background: 'var(--sg-frost, #F7F6F3)' }}>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-px" style={{ background: 'linear-gradient(to right, transparent, rgba(201,168,76,0.15), transparent)' }} />
    </section>
  )
}
