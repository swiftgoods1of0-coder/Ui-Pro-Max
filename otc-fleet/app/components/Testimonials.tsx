        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {testimonials.map((t, i) => (
            <FadeUp key={i} delay={i * 0.15}>
              <div className="relative bg-navy-800 border border-white/10 rounded-lg p-8 h-full flex flex-col hover:border-[#f97316]/40 transition-colors duration-300">

                {/* Quote icon */}
                <div className="absolute top-6 right-8 opacity-10">
                  <Quote className="w-16 h-16 text-[#f97316]" />
                </div>

                {/* Stars */}
                <div className="flex gap-1 mb-6">
                  {Array.from({ length: t.rating }).map((_, s) => (
                    <Star key={s} className="w-5 h-5 fill-orange-500 text-orange-400" />
                  ))}
                </div>
          ...
          <div className="mt-12 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                ...
                className="inline-flex items-center gap-2 px-6 py-3 bg-orange-500 text-white font-bold rounded hover:bg-orange-500-dark transition-colors duration-200 text-sm tracking-wide"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 border border-[#f97316] text-[#f97316] font-bold rounded hover:bg-[#f97316]/10 transition-colors duration-200 text-sm tracking-wide"
              >
                <Star className="w-4 h-4" />
